...
namespace Swift
{
    ...
    public class StateMachineCombination : StateMachine
    {
        ...
        {
            base.Start();
            sms.ToList().ForEach((sm) => { sm.Start(); });
        }

        // 销毁状态机，不能再次启动
        public override void Destroy()
        {
            sms.ToList().ForEach((sm) => { sm.Destroy(); });
            base.Destroy();
        }

        public override void Pause()
        {
            sms.ToList().ForEach((sm) => { sm.Pause(); });
            base.Pause();
        }

        public override void Resume()
        {
            sms.ToList().ForEach((sm) => { sm.Resume(); });
            base.Resume();
        }

        public override void Run(Number te)
        {
            sms.ToList().ForEach((sm) => { sm.Run(te); });
            base.Run(te);
        }
        ...
        public override void LogAllStates(Action<string> logger)
        {
            ...
            sms.ToList().ForEach((sm) => { sm.LogAllStates(logger); });
        }
